...
func TestWhatsAppSendText(t *testing.T) {
	...
	client := NewClientWithConfig(Config{APIKey: "test-key", BaseURL: server.URL, AllowInsecureHTTP: true})
...
func TestWhatsAppSendError(t *testing.T) {
	...
	client := NewClientWithConfig(Config{APIKey: "k", BaseURL: server.URL, AllowInsecureHTTP: true})
