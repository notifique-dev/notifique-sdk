...
public class PushApiTests
{
    ...
    private const string SendPushJson = "{\"success\":true,\"data\":{\"status\":\"QUEUED\",\"count\":2,\"messageIds\":[\"push-1\",\"push-2\"]}}";
    ...
    public async Task SendMessageAsync_Success()
    {
        ...
        var request = new SendPushRequest
        {
            ...
            Type = "push",
            Payload = new Dictionary<string, object> { ["title"] = "Hello", ["body"] = "World" },
        };
        var response = await _client.Push.SendMessageAsync(request);
        Assert.True(response.Success);
        Assert.Equal("QUEUED", response.Data.Status);
        Assert.Equal(2, response.Data.Count);
        Assert.Equal(new[] { "push-1", "push-2" }, response.Data.MessageIds);
    }

    [Fact]
    public async Task SendMessageAsync_WithSchedule_SendsSendAt()
    {
        _handler.SetResponse(HttpStatusCode.OK, "{\"success\":true,\"data\":{\"status\":\"SCHEDULED\",\"count\":1,\"messageIds\":[\"p1\"],\"scheduledAt\":\"2025-12-31T14:00:00.000Z\"}}");
        var request = new SendPushRequest
        {
            To = new List<string> { "dev-1" },
            Type = "push",
            Payload = new Dictionary<string, object> { ["title"] = "Scheduled" },
